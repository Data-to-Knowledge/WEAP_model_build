...
def get_CRC_DB(config):
    ...
    #-Get all the WAPs that are within one of the selected SWAZs from the D_SW_WellsDetails table and update locations with Well_StreamDepletion_Locations where required
    ...
    SWAZ_WAPs_withSD = None; SWAZ_WAPs_withSD2 = None; SWAZ_WAPs_withSD3 = None; del SWAZ_WAPs_withSD, SWAZ_WAPs_withSD2, SWAZ_WAPs_withSD3
    ...
    #-calculate Connectivity using 7 days of pumping
    pump_days = 7
    sd_df['Connection7'] = np.nan
    for i in sd_df.iterrows():
        distance, T, S = i[1]['Distance'], i[1]['T_Estimate'], i[1]['S']
        y = Theis(T, S, distance, 0, pump_days)
        connection7 = y[1]
        sd_df.loc[i[0],'Connection7'] = connection7
    sd_df.reset_index(inplace=True)

    #-calculate Connectivity using 30 days of pumping
    pump_days = 30
    sd_df['Connection30'] = np.nan
    for i in sd_df.iterrows():
        distance, T, S = i[1]['Distance'], i[1]['T_Estimate'], i[1]['S']
        y = Theis(T, S, distance, 0, pump_days)
        connection30 = y[1]
        sd_df.loc[i[0],'Connection30'] = connection30
    sd_df.reset_index(inplace=True)

    #-calculate Connectivity using 150 days of pumping
    pump_days = 150
    sd_df['Connection150'] = np.nan
    for i in sd_df.iterrows():
        distance, T, S = i[1]['Distance'], i[1]['T_Estimate'], i[1]['S']
        y = Theis(T, S, distance, 0, pump_days)
        connection150 = y[1]
        sd_df.loc[i[0],'Connection150'] = connection150
    sd_df.reset_index(inplace=True)

    #-merge sd dataframes
    df1 = pd.merge(df1, sd_df[['wap', 'wap_sd_NZTMX', 'wap_sd_NZTMY', 'Distance','T_Estimate','S', 'Connection', 'Connection7', 'Connection30', 'Connection150']], how='left', on='wap')
    ...
    df_final = df1[['crc', 'B1_APPL_STATUS', 'fmDate', 'toDate', 'Given Effect To', 'HolderAddressFullName', 'Activity', 'use_type', 'irr_area[ha]', 'from_month', 'to_month', 'SWAZ',
                ...
                'wap_return_period [d]', 'wap_NZTMX', 'wap_NZTMY', 'wap_sd_NZTMX', 'wap_sd_NZTMY', 'Distance', 'T_Estimate','S', 'Connection', 'Connection7', 'Connection30', 'Connection150', 'discharge_rate [l/s]',
