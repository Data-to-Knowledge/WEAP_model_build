...
def initCatchmentTable(config):
    ...
    catchmentDBF = os.path.join(catchmentDir, config.get('CATCHMENTS', 'catchmentDBF'))
    riverbedDBF = os.path.join(catchmentDir, config.get('CATCHMENTS', 'riverbedDBF'))
    catchmentIDs = os.path.join(catchmentDir, config.get('CATCHMENTS', 'catchmentIDs'))
    
    #-read selected sub-catchment IDs
    id_df = pd.read_csv(catchmentIDs)
    #-read sub-catchment dbf
    dbf = Dbf5(catchmentDBF)
    df = dbf.to_dataframe()
    ...
    subcatch_df.drop(['CenterX', 'CenterY','CenterX_riverbed', 'CenterY_riverbed'], axis=1, inplace=True)
...
def addRiverbedCatchments(WEAP, catch_df):
    ...
    riverbed_df = catch_df.loc[catch_df['Riverbed area km2']>0]  #-remove any records with zero area riverbed
...
def setRiverbedCatchmentProps(WEAP, catch_df, catchmentDir):
    ...
    kc_df = pd.read_csv(os.path.join(catchmentDir, 'Kc_rb.csv'), parse_dates=True, dayfirst=True, index_col=0)
    ...
    ids = list(pd.read_csv(os.path.join(catchmentDir, 'Precipitation_rb.csv'), parse_dates=True, dayfirst=True, index_col=0).columns.astype(np.int))
    for ID in ids:
        ...
        if WEAP.Branch(s):
            ...
            WEAP.Branch(s).Variables('Method').Expression = 1  # "Rainfall Runoff (simplified coefficient method)"
            ...
            #-Set the catchment area - NOTE: unit has to be set manually otherwise it doesn't work            
            area = riverbed_df.loc[ID, 'Riverbed area km2']
            WEAP.Branch(s).Variables('Area').Expression = area
            
        else:
            print 'ERROR: Catchment_%s is not present, and therefore properties cannot be set'
            print 'Automation procedure was stopped.'
            WEAP.SaveArea()
            WEAP.Verbose=1
            exit(0)
...
def postprocessSPHY(config, catchmentDir, catch_df, selSdate, selEdate):
    ...
    #-number of catchments (columns) for which a time-series has been created in the tss file    
    nrCatchments = config.getint('CATCHMENTS', 'nrCatchments')
    #-number of riverbed catchments (columns) for which a time-series has been created in the tss file -> does not have to be equal to nr of Catchments    
    ...
    #-Dictionary with catchment variables to convert time-series for (non-riverbed area)
    catchmentVarDict = {'Precipitation': 'subcatch_PrecTSS.tss', 'ETr': 'subcatch_ETRTSS.tss', 'ETp': 'subcatch_ETPTSS.tss', 'ETa': 'subcatch_ETATSS.tss',
               'SurfaceRunoff': 'subcatch_RootRunoffTSS.tss', 'Drainage': 'subcatch_RootDrainTSS.tss', 'GwRecharge': 'subcatch_GWRechargeTSS.tss',
               'Baseflow': 'subcatch_BaseRTSS.tss', 'SnowRunoff': 'subcatch_SnowRTSS.tss', 'GlacierRunoff': 'subcatch_GlacRTSS.tss', 'Kc': 'subcatch_KcTSS.tss'}
    
    #-Dictionary with riverbed catchment variables to concert time-series for
    riverbedVarDict = {'Precipitation_rb': 'riverbed_PrecTSS.tss', 'ETr_rb': 'riverbed_ETRTSS.tss', 'ETp_rb': 'riverbed_ETPTSS.tss', 'ETa_rb': 'riverbed_ETATSS.tss',
               'Kc_rb': 'riverbed_KcTSS.tss'}
    ...
    for k in keys:
        ...
        f = pd.read_csv(os.path.join(catchmentDir, catchmentVarDict[k]), delim_whitespace=True, skipinitialspace=True, skiprows=3+nrCatchments, header=None, index_col=False)
        ...
        #-reset index and re-format Date column to dd/mm/yyyy, otherwise WEAP doesn't understand it. Day first.
        f.reset_index(inplace=True)
        f['Date'] = f['Date'].dt.strftime('%d/%m/%Y')
        #-Write to csv-file
        f.to_csv(os.path.join(catchmentDir, k + '.csv'), index=False)

    #-Create a csv-file for the specific runoff [MCM per day] of the non-riverbed catchment area
    surface_runoff = pd.read_csv(os.path.join(catchmentDir, 'SurfaceRunoff.csv'),parse_dates=True, dayfirst=True, index_col=0)
    drainage = pd.read_csv(os.path.join(catchmentDir, 'Drainage.csv'),parse_dates=True, dayfirst=True, index_col=0)
    snowr = pd.read_csv(os.path.join(catchmentDir, 'SnowRunoff.csv'),parse_dates=True, dayfirst=True, index_col=0)
    glacr = pd.read_csv(os.path.join(catchmentDir, 'GlacierRunoff.csv'),parse_dates=True, dayfirst=True, index_col=0)
    basf = pd.read_csv(os.path.join(catchmentDir, 'Baseflow.csv'),parse_dates=True, dayfirst=True, index_col=0)
    Qspec = (surface_runoff + drainage + basf + snowr + glacr) / 1000 #-in m per day
    #-Convert mm/day to MCM/day
    landArea = catch_df[['Land area km2']].transpose() * 1000000  #-Calculate land area in m2
    Qspec = Qspec.multiply(landArea.iloc[0].values, axis=1) #-specific runoff in m3/day
    ...
    ids = sorted(ids)
    #-define columns to use the catchment ID that corresponds with the river bed ID
    coldict = {}
    for ID in ids:
